...
#define BUF_SIZE 256 
#define FADE_SAMPLES 120 // ~3.7ms fade-in to prevent the "pop" at start
...
struct Voice {
  ...
  int col = -1;
  bool active = false;
  uint32_t samplesPlayed = 0; // Tracks progress for the soft-start fade
};
...
void setup() {
  ...
  if (!SD.begin(SD_CS, SPI, 40000000)) { while(1); } // 40MHz for multi-note speed
...
void loop() {
        ...
        for (int i = 0; i < MAX_VOICES; i++) {
          if (!voices[i].active) {
            ...
            if (voices[i].file) {
              ...
              voices[i].samplesPlayed = 0; // Prepare for fade-in
  ...
  for (int i = 0; i < MAX_VOICES; i++) {
    if (voices[i].active && voices[i].file.available()) {
      ...
      for (int j = 0; j < samplesRead; j++) {
        // Soft-Start Logic: Smoothly ramp volume from 0 to 1
        float vol = 1.0;
        if (voices[i].samplesPlayed < FADE_SAMPLES) {
          vol = (float)voices[i].samplesPlayed / FADE_SAMPLES;
        }

        int32_t sample = (int32_t)tempBuf[j] * vol;
  ...
  if (anyActive) {
    ...
    i2s_zero_dma_buffer(I2S_NUM_0); // Prevents buzzing/hissing
  }
}
